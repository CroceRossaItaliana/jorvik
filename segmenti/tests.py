...
class TestSegmenti(TestCase):
    ...
    def test_appartenenza(self):
        ...
        # Notizie di test
        notizia_1 = Articolo.objects.create(
            titolo="Notizia 1",
            corpo="Testo di prova!",
            stato=Articolo.PUBBLICATO
        )
        notizia_2 = Articolo.objects.create(
            titolo="Notizia 2",
            corpo="Testo di prova!",
            stato=Articolo.PUBBLICATO
        )
        ...
        segmento_tutti_no_filtri = ArticoloSegmento.objects.create(
            segmento='A',
            articolo=notizia_1
        )
        ...
        segmento_volontari_meno_uno_no_filtri = ArticoloSegmento.objects.create(
            segmento='C',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i volontari con più di un anno di attivita
        segmento_volontari_piu_uno_no_filtri = ArticoloSegmento.objects.create(
            segmento='D',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i volontari con meno di 35 anni
        segmento_volontari_meno_35_anni_no_filtri = ArticoloSegmento.objects.create(
            segmento='E',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i volontari con meno di 35 anni
        segmento_volontari_35_anni_o_piu_no_filtri = ArticoloSegmento.objects.create(
            segmento='F',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i sostenitori
        segmento_sostenitori_no_filtri = ArticoloSegmento.objects.create(
            segmento='G',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i sostenitori
        segmento_aspiranti_corsisti_no_filtri = ArticoloSegmento.objects.create(
            segmento='H',
            articolo=notizia_1
        )
        ...
        segmento_presidenti_comitati_locali_no_filtri = ArticoloSegmento.objects.create(
            segmento='J',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i presidenti di comitati regionali con delega attiva
        segmento_presidenti_comitati_regionali_no_filtri = ArticoloSegmento.objects.create(
            segmento='K',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i delegati Ufficio Soci con delega attiva
        segmento_delegati_US_no_filtri = ArticoloSegmento.objects.create(
            segmento='L',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i delegati Autoparco con delega attiva
        segmento_delegati_autoparco_no_filtri = ArticoloSegmento.objects.create(
            segmento='Y',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i delegati Formazione con delega attiva
        segmento_delegati_formazione_no_filtri = ArticoloSegmento.objects.create(
            segmento='Z',
            articolo=notizia_1
        )
        # Segmento per filtrare tutti i volontari con titolo
        titolo_patenteCRI = Titolo.objects.create(tipo='PC', nome='Titolo test')
        segmento_volontari_con_titolo_no_filtri = ArticoloSegmento.objects.create(
            segmento='AA',
            articolo=notizia_1,
        ...
        for livello_obiettivo in LIVELLI_DELEGATI:
            ...
            segmento = ArticoloSegmento.objects.create(
